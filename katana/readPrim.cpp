        ...
        const UsdPrim& rootPrim)
{
    ...
    for (auto aovPrim : aovPrims)
    {
        ...
        std::pair<decltype(aovNames)::iterator, bool> insertResult = aovNames.insert(name);
        if (!insertResult.second) {
            continue;
        }

        // TODO: Should we allow these through and just use our defaults?
        const UsdShadeShader driver = getAOVRelationshipTarget(aov.GetDriverRel());
        if (!driver) {
            TF_WARN("Invalid driver relationship target on AiAOV %s",
                    aovPrim.GetPath().GetText());
            continue;
        }
        const UsdShadeShader filter = getAOVRelationshipTarget(aov.GetFilterRel());
        if (!filter) {
            TF_WARN("Invalid filter relationship target on AiAOV %s",
                    aovPrim.GetPath().GetText());
            continue;
        }
        ...
        if (driverId == driver_exr) {
            static FnKat::GroupAttribute exrDriverTemplate(
                    "compression", FnKat::StringAttribute("zips"),
                    "half_precision", FnKat::IntAttribute(1),
                    "tiled", FnKat::IntAttribute(0),
                    "autocrop", FnKat::IntAttribute(1),
                    false);
        ...
        // Filter params
        FnKat::GroupBuilder filterBuilder(FnKat::GroupBuilder::BuilderModeStrict);
        ...
        channelBuilder.set(
                "name", nameAttr).set(
                "channel", nameAttr).set(
                "driver", FnKat::StringAttribute(driverId.GetText())).set(
                "filter", FnKat::StringAttribute(filterId.GetText())).set(
                "type", FnKat::StringAttribute(dataType.GetText())).set(
                "driverParameters", driverBuilder.build());
        ...
        if (aov.GetLPEAttr().Get<std::string>(&lightPathExpression)) {
            channelBuilder.set("lightPathExpression",
                               FnKat::StringAttribute(lightPathExpression));
        }

        static const std::string channelAttrPrefix("arnoldGlobalStatements.outputChannels.");
        interface.setAttr(channelAttrPrefix + name,
                          channelBuilder.build(), false);
        ...
        interface.setAttr(
                ...
                FnKat::GroupAttribute(
                    "rendererSettings", FnKat::GroupAttribute("channel", nameAttr, false),
                    "locationType", FnKat::StringAttribute("file"),
                    false),
                false);
    }
}
