...
size_t applyProceduralArgsAttrs(
    ...
    for (const auto& userAttr : nodeAPI.GetUserAttributes()) {
        VtValue vtValue;
        if (!userAttr.Get(&vtValue, time)) { continue; }
        ...
        std::string typeHint =
            getArnoldAttrTypeHint(userAttr.GetTypeName().GetScalarType());
        if (!typeHint.empty()) {
...
void getArnoldVDBVolumeOpArgs(
    ...
    static FnKat::GroupAttribute fixedDefaults(
        "grids", FnKat::StringAttribute(), "velocity_grids",
        FnKat::StringAttribute(), "override_motion_range",
        FnKat::IntAttribute(0), "makeInteractive", FnKat::StringAttribute("No"),
        true);

    static const std::vector<OptionalAttributeDefinition<float, UsdAiVolumeAPI>>
        floatAttrs = {
            ...
            {&UsdAiVolumeAPI::GetAiVelocityOutlierThresholdAttr,
             "velocity_outlier_threshold", 0.001f},
        };

    static const std::vector<OptionalAttributeDefinition<bool, UsdAiVolumeAPI>>
        boolAttrs = {
    ...
    if (volumeAPI) {
        handleAttributes(
            floatAttrs, volumeAPI, argsBuilder, /*applyDefaults=*/true);
        handleAttributes(
            boolAttrs, volumeAPI, argsBuilder, /*applyDefaults=*/true);
    } else {
        ...
        for (const auto& entry : floatAttrs) {
            argsBuilder.set(
                entry.paramName, createAttribute(entry.defaultValue));
        }
    ...
    if (auto basisCurves = UsdGeomBasisCurves(prim)) {
        ...
        if (type == UsdGeomTokens->linear) {
            ...
            if (basis == UsdGeomTokens->bezier) {
                ...
                TF_WARN(
                    "Can't translate basis type %s on %s, falling back to "
                    "linear.",
...
void updateOrCreateAttr(
    ...
    if (attr.getType() == kFnKatAttributeTypeGroup) {
        FnAttribute::GroupAttribute existingAttr =
            interface.getOutputAttr(attrName);
        if (existingAttr.isValid()) {
            interface.setAttr(
                attrName, FnAttribute::GroupBuilder()
                              .update(existingAttr)
                              .deepUpdate(attr)
                              .build());
        } else {
