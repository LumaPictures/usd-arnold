...
TEST(UsdAiShaderExport, ParameterConnections) {
    ...
    EXPECT_TRUE(validateConnection(api, "Kr", "/Looks/image2.outputs:b"));
    EXPECT_TRUE(validateConnection(api, "Kr_color", "/Looks/image2.outputs:out"));
    EXPECT_TRUE(validateConnection(api, "Kt", "/Looks/image1.outputs:g"));
    EXPECT_TRUE(validateConnection(api, "Kt_color", "/Looks/image1.outputs:out"));
    EXPECT_TRUE(validateConnection(api, "transmittance:r", "/Looks/image2.outputs:r"));
    EXPECT_TRUE(validateConnection(api, "transmittance:b", "/Looks/image1.outputs:g"));
}
